...
* Submission package details:
  1. The vocabulary file of input, which includes all possible input token types and special tokens (`<sos>`, `<eos>`, `<blank>`, etc). In the file, each line contains a single token. E.g. if bpe is used, this file is `data/token_list/src_bpe_unigram3000_rm_wavlm_largge_21_km2000/bpe.vocab` following the baseline framework. If bpe is not used, this file is `data/token_list/char/tokens.txt`. Example output:
    ```
    <unk>   0
    <s>     0
    </s>    0
