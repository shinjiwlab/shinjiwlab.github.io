...
2. Add depndencies for a long-run job

In many cases, our training cannot be finishing in 2 days. However, since PSC has the restriction that no jobs can exceeding 2 days during submission, we can start a job with dependency for long jobs. For example, you already start a job ID is 000001 and you want a following job right after it. You can submit jobs like:
```
