...
* Data copying / file transfer
  * Suggest to use `data.bridges2.psc.edu` as the machine name. Doing file transfer with `rsync`, `sftp`, `scp`, etc. [Detailed usage info](https://portal.xsede.org/psc-bridges-2#filetransfer-copybridgesfiles). Following is an example of `scp`.
    ```
